...
- Macros: options-before-handler arg order works; `beforeHandle` isn't suggested by autocomplete but type-checks; macro's own derive output isn't typed in its beforeHandle ctx (annotate the destructured param) — but IS typed correctly in route handler ctx.
