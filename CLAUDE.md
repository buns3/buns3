...
DONE: object storage (streaming, atomic, metadata), bucket lifecycle, API-key auth end-to-end, Elysia 2.0 port (3c2119a), OpenAPI admin docs + bucket object counts (4ef9f81), createdAt tz fix — JS-stamped timestamps, DB defaults dropped (23f5c09), public-read bucket flag end-to-end incl. anonymous fallback + PATCH endpoint (62c24ab; full 15-case curl matrix green), upload tester page — now versioned at `tools/upload-tester.html` (React UMD + Babel standalone PINNED to @7: v8's react preset emits automatic-runtime imports that break classic script tags; upload with explicit `Content-Type: text/html` or the no-sniffing rule serves it as a download).

NEXT (in order):
1. **Upstream bug reports** (receipts in git/transcripts): Elysia — Content-Range on 200s (visible in the tester's log on any GET; "undefined" on empty files), Standard Schema messages dropped in schema-slot validation, macro beforeHandle autocomplete, macro-derive union collapsing ctx types; @elysia/openapi — broken dist/gen typebox imports (patched locally), RegExp exclude ignored, 1.4.15's uncapped peer range; Prisma Next — rendered `now()` default doesn't round-trip through its own `sqlite/datetime@1` codec.
2. Perennials: test files (validators/encoders/keyStorage round-trip — all edge cases already known from probes), README (API + documented decisions: deny-by-default publicRead, uniform anonymous 401, presented-key-always-evaluated, PATCH ≥1 property, createdAt = current version's write time — overwrite moves it), phase 4: presigned URLs (HMAC over method+bucket+key+expiry, query-param auth path in the macro).
